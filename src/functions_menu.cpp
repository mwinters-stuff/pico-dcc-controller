...
FunctionsMenu::FunctionsMenu(std::shared_ptr<DisplayControls> displayControls): MuiMenu(displayControls), menu(){
...
void FunctionsMenu::setLoco(DCCExController::Loco* loco) { 
  ...
  //   menuItem item;
  //   item.value = i + 1;
  //   item.label = (loco->getFunctionName(i + 1) ? "F" : "f") + std::to_string(i + 1);
  //   menu.push_back(item);
  // }
...
void FunctionsMenu::buildMenu(u8g2_t &u8g2) {
  // create root page
  muiItemId root_page = makePage("Functions");  // provide a label for the page
...
void FunctionsMenu::clearAction(){
